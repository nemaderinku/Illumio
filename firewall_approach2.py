...
class firewall:
    def __init__(self, filename):
        ...
        self.dict = {"inbound": {"tcp": [], "udp": []}, "outbound": {"tcp": [], "udp": []}}
        for i in self.df:
            curr = self.df.iloc[i]
            new_list = self.dict[str(curr[0])][str(curr[1])]
            new_list.append(list(curr))
            self.dict[str(curr[0])][str(curr[1])] = new_list
    def accept_packet(self, direction, protocol, port, ip_address):
        if direction in self.dict and protocol in self.dict[direction]:
            rule_list = self.dict[direction][protocol]
            for i in rule_list:
                rule_port = str(i[2])
                rule_ports = None
                ...
                if rule_ports and int(rule_ports[0]) <= int(port) <= int(rule_ports[1]):
                    port_test = True
                elif (not rule_ports) and int(port) == int(rule_port):
                    port_test = True
                if port_test:
                    if rule_ips:
